...
public class AuthConsumer {
	...
	public void processApiRequest(ApiRequestData request, @Header(AmqpHeaders.CORRELATION_ID) String correlationId) {
		...
		response.setStatusCode(200); // 기본 상태 코드 설정 (문제 발생 방지용)
		...
		try {
			switch (requestKey) {
				case "GET /api/auth/test" -> processMono = authService.test(request, response);
				case "GET /api/auth/health" -> processMono = authService.health(request, response);
//				case "POST /api/auth/google/android" -> processMono = authService.processOAuthRequest(request, response);
				default -> {
