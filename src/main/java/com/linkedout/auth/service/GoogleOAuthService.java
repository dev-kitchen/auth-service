  ...
  public AuthResponse loginOrSignup(GoogleUserInfo userInfo) {
    ...
    AccountDTO account =
        serviceMessageClient
            .sendMessage("account", "findByEmail", userInfo.getEmail(), AccountDTO.class)
            .doOnNext(existingAccount -> log.info("계정 조회 결과: {}", existingAccount))
            .switchIfEmpty(
                Mono.defer(
                    () -> {
                      log.info("계정이 존재하지 않음. 새로 생성합니다.");
                      return serviceMessageClient.sendMessage(
                          "account", "createAccount", userInfo, AccountDTO.class);
                    }))
